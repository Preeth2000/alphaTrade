...
**Architecture:** New methods on `T212Client` cover stop/limit/get/cancel endpoints. A standalone async function `monitor_oco` in `alphaTrade/broker/oco_monitor.py` polls both orders and closes the position in SQLite when one fills. `main.py` launches the monitor as an asyncio task immediately after each BUY fill.
...
|---|---|---|
| Modify | `alphaTrade/broker/t212_client.py` | Add `place_stop_order`, `place_limit_order`, `get_order`, `cancel_order`, `_delete` |
| Create | `alphaTrade/broker/oco_monitor.py` | Async OCO polling loop |
| Modify | `alphaTrade/main.py` | Launch OCO task after BUY fill |
...
Open `alphaTrade/broker/t212_client.py`. After the `_post` method (line 32), add:
...
```bash
git add alphaTrade/broker/t212_client.py tests/unit/test_t212_client_oco.py
...
**Files:**
- Create: `alphaTrade/broker/oco_monitor.py`
...
Expected: `ModuleNotFoundError: No module named 'alphaTrade.broker.oco_monitor'`
...
```bash
git add alphaTrade/broker/oco_monitor.py tests/unit/test_oco_monitor.py
...
At the top of `alphaTrade/main.py`, add to the first-party imports block (keep alphabetical):
...
In `alphaTrade/main.py`, find the BUY branch (around line 287). The current block is:
...
```bash
bd close alphaTrade-h6c --reason="OCO SL/TP implemented: T212Client stop/limit/get/cancel, oco_monitor.py, main.py BUY branch wired, tests added"
```
